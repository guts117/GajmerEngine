...
struct Model_Importer::Impl
{
	...
	void LoadMaterials(const aiScene* scene)
	{
		...
		//			auto texture = new Texture(texPath, true);
		...
		//	}
			...
			//TextureMap = RenderEngineMain::CreateTextureMap();
		//}
	}
...
Model_Importer::~Model_Importer() = default;

void Model_Importer::LoadModel(std::string&& fileName) 
{
	Pimpl()->LoadModel(std::move(fileName));
}
