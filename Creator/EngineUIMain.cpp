...
struct EngineUIMain::Impl
{
	...
	std::vector<WorldViewer> m_WorldList;
	...
	Impl(const bool installCallbacks, const std::string version) noexcept
		: m_WorldList{ std::vector<WorldViewer>() }
...
;
	...
	{
		auto World = WorldViewer(WorldTex, WorldName, viewerType, selectCallback);
...
void EngineUIMain::AddWorldViewers(GLuint WorldTex, std::string WorldName, WorldViewerType viewerType, std::function<void(bool)> selectCallback)
{
	Pimpl()->AddWorldViewers(WorldTex, WorldName, viewerType, selectCallback);
}
