...
void Directional_Shadow_Map_Render_Pass_Handler::Update(std::vector<std::vector<Render_Object>>& renderObj, const CamParam* camParam, const LightParam* lightParam)
{
	...
	for (size_t cascadeId = 0; cascadeId < NUM_CASCADES; ++cascadeId)
	{
		...
		for (auto shaderIndex = 0; shaderIndex < m_shaderVec.size(); ++shaderIndex)
		{
			...
			shader->SetVariable("DirectionalLightPose", lightParam->Projection[cascadeId] * lightParam->View[cascadeId]);
