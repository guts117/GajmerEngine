...
GraphicsMain::GraphicsMain(const glm::ivec2& screenDims) : m_pImpl{ std::make_unique<Impl>() } { Pimpl()->Init(screenDims); };

void GraphicsMain::Update(const glm::ivec2& screenDims, const bool& isUpdateBuffers)
{	
	Pimpl()->Update(screenDims, isUpdateBuffers);
}

void GraphicsMain::EndUpdate() 
{
	Pimpl()->EndUpdate();
}
...
GraphicsMain::~GraphicsMain() = default;